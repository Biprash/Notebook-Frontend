import { ChangeEvent, Dispatch, FormEvent, FormEventHandler, ReactElement, SetStateAction, useState } from 'react'

interface Props {
    setShowSectionForm: Dispatch<SetStateAction<boolean>>
}

function SectionForm({setShowSectionForm}: Props): ReactElement {
    const [title, setTitle] = useState<string>('')

    const handleSectionFormSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        console.log('submit for section is on todo.');
    }
    return (
